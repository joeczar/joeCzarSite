;(function() {
  ...
  savedGames.on("mouseover", "div", e => {
    $(e.target).addClass("highlight")
  })
  savedGames.on("mouseout", "div", e => {
  ...
  function loadGames() {
    ...
    var html = ""
    if (games.length === 0) {
      html += '<div class="saved-game">No Games Saved...</div>'
    } else {
      html += games
        .map(
          (game, i) =>
            `<div class="saved-game" id=${i}>${game.name} ${game.score} ${game.date}</div>`
        )
        .join("")
    }
    savedGames.html(html)
  ...
  function saveGame() {
    ...
    var saveObj = {
      name: `${game.player1.name} vs. ${game.player2.name}`,
      score: `${game.player1.score} to ${game.player2.score}`,
      game: save,
